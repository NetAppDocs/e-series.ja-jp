...
RHEL 7 および SLES 12 では、 NVMe over Fibre Channel を使用するマルチパスに Device Mapper Multipath （ DMMP ）を使用します。RHEL 8 および SLES 15 では、ネイティブの NVMe フェイルオーバーが組み込まれています。実行している OS に応じて、適切に実行するためにマルチパスを追加で設定する必要があります。
...
== RHEL 8 にネイティブ NMVe マルチパスをセットアップします
